...
import {MyCacheService} from 'src/my-cache/my-cache.service';
...
export class GameService {
  ...
  private BALL_SPEED_X: number = this.SCREEN_HEIGTH * 0.012;
  private BALL_SPEED_Y: number = this.SCREEN_HEIGTH * 0.012;
  ...
  async setBallRandomDirection(gameRoomKey: string): Promise<Vector2> {
    ...
    return {
      X:
        Math.random() > 0.5
          ? this.SCREEN_WIDTH * 0.008
          : this.SCREEN_WIDTH * -0.008,
      Y: this.SCREEN_HEIGTH * 0.02,
    };
  }
